    ...
    function renderTable() {
        ...
        filtered.forEach(function (employee) {
            ...
            row.innerHTML = `
                ...
                <td><span class="${employee.isActive ? 'status-active' : 'status-inactive'}">${employee.isActive ? 'Active' : 'Inactive'}</span></td>
                <td class="action-cell">
                    <a href="hr_employee_view.html?employee_id=${employee.id}" class="action-link" title="View"><i class="fas fa-eye"></i></a>
                    <span class="separator">|</span>
                    <a href="hr_employee_edit.html?employee_id=${employee.id}" class="action-link" title="Edit"><i class="fas fa-pen"></i></a>
                    <span class="separator">|</span>
                    <button type="button" class="action-link" title="Delete" style="background:none; border:none; color:#a02d2d; cursor:pointer; padding:0; font-size:inherit;">
            ...
            if (deleteBtn) {
                deleteBtn.addEventListener('click', async function () {
                    if (!confirm(`Deactivate ${employee.fullName || 'this employee'}?`)) return;
                    const response = await fetch(`/api/employees/${encodeURIComponent(employee.id)}`, { method: 'DELETE' });
                    if (!response.ok) {
                        alert('Unable to deactivate employee record.');
                        return;
                    }
                    await loadEmployees();
                });
